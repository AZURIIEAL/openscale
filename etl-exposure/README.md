...
data/raw/ (downloaded from TLC, 2023-02 → 2026-05, 40 months)
  ...
  4 aggregates                 │                                (Spark Structured Streaming,
        │                      │                                 windowed hourly aggregates)
        ▼                      │
...
| 7 – Streaming (Kafka) | Done (v2) — consumer now runs on real Spark Structured Streaming |
| 8 – Feature Store | Done (v1) |
| 9 – ML | Done (v1, fare prediction only) |
| 10 – API | Done (v1) |
| 11 – Observability | Done (v2, metrics + Grafana dashboards — no OpenTelemetry tracing) |
...
streaming/
  ...
  spark-streaming/        Real Spark Structured Streaming consumer, windowed hourly aggregates
feature-store/
...
observability/
  ...
  grafana/                Datasource + dashboard provisioning (OpenScale API dashboard)
...
Open Grafana at http://localhost:3000 — the "OpenScale API" dashboard (folder: OpenScale) is auto-provisioned and loads on startup: target up/down, request rate, error rate, and latency percentiles (P50/P95/P99, overall and per-endpoint), all against live Prometheus data. Hit a few API endpoints first if the panels look empty — there's nothing to show until requests actually happen.
...
Needs the same PySpark env vars as the batch jobs (see PySpark setup above) since the consumer is a real Spark job.
...
# In another terminal: Spark Structured Streaming consumer, prints windowed
# hourly trip-count/revenue aggregates every 10s micro-batch
python "streaming/spark-streaming/consumer.py" --starting-offsets earliest --timeout-seconds 60
```

## Known issues / honesty notes

- **No consumer-side persistence.** The streaming consumer prints windowed aggregates to console; nothing writes them anywhere durable yet (no Gold-equivalent streaming output table).
