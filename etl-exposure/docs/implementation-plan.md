...
| Last updated | 2026-08-09 (Phase 7 streaming consumer ported to real Spark Structured Streaming) |
...
| Streaming | **Done (v2, real Spark Structured Streaming)** | `streaming/replay-service/replay.py` reads a day of Silver trips from MinIO (via Spark, filtered before `.toPandas()`) and publishes them to Kafka as JSON, sorted by pickup time, with an optional `--speed` pacing multiplier (default fast-forward). `streaming/spark-streaming/consumer.py` now runs a real `spark.readStream.format("kafka")` job with a 1-hour watermark and tumbling-window `groupBy(window(...)).agg(...)`, printing updated trip-count/revenue aggregates to the console every micro-batch. This closes the last "not really Spark despite the name" gap. Verified end-to-end: replayed 3,000 real trips, consumer correctly produced hourly windowed counts/revenue. See `docs/issues-and-fixes.md` #13 for the two bugs hit (and fixed) getting here. |
...
**Not done in this pass:** `spark/silver/validator.py` is still pandas (fine — bounded per-month memory regardless of how many months exist, no wall to hit there). The Phase 7 streaming consumer wasn't ported to Spark Structured Streaming in this pass either — that landed separately on 2026-08-09, see Phase 7 below.
...
## Phase 7 — Streaming *(done, v2 — now real Spark Structured Streaming)*

- `streaming/kafka-producer/producer.py` — `get_producer()`/`send_trip()` helpers, JSON-serialized, topic `yellow-taxi-trips`
- `streaming/replay-service/replay.py` — loads one pickup-date's worth of Silver trips (via Spark, filtered before `.toPandas()` — fixed alongside this change, see below), sorts by pickup time, publishes each as a JSON event (`--speed` controls real-time pacing; `--limit` caps volume for testing)
- `streaming/spark-streaming/consumer.py` — **real `spark.readStream.format("kafka")`** job now (was plain `kafka-python` + an in-memory dict). Parses each event against an explicit schema matching Silver trips, applies a 1-hour watermark, and maintains **tumbling 1-hour windowed aggregates** (trip count + revenue) via `groupBy(window(...)).agg(...)`, printing updated windows to the console every 10s micro-batch.

**What changed to make this real:**
- `spark/common/spark_session.py`'s `get_spark()` gained an `extra_packages` parameter so the consumer can add `org.apache.spark:spark-sql-kafka-0-10_2.13:4.2.0` (version-matched to `spark-core_2.13-4.2.0.jar`, same concern as `HADOOP_AWS_PACKAGE`) without every batch job paying for Kafka connector resolution.
- Structured Streaming needs a checkpoint directory; the consumer uses a local (not MinIO) `streaming/spark-streaming/.checkpoints/` dir, gitignored, and clears it on every run since this is a rerunnable local demo, not a deployment that should resume from a prior offset.
- **Bug found and fixed along the way:** `replay.py`'s `load_day()` was still calling `load_silver_trips()` with no arguments — a leftover from before Phase 5 that was never updated when `load_silver_trips(spark)` started requiring a `SparkSession`. It would have raised `TypeError` on the very next run. Fixed by creating a Spark session in `replay.py`, filtering to the requested day in Spark, and only then `.toPandas()` — same big-in-Spark/small-in-pandas pattern as everywhere else in the Phase 5 migration.

**Known limitations, not yet done:**
- `replay.py` operates on one `--date` at a time, not a continuous multi-day stream.
- No consumer-side persistence — windowed aggregates print to console and aren't written anywhere durable (no Gold-equivalent streaming output table yet).
- No idle-based auto-stop the way the old `consumer_timeout_ms` gave the kafka-python version — `--timeout-seconds` just bounds total run time; a real deployment would run indefinitely (Ctrl+C to stop) instead.
...
## Phase 11 — Observability *(done, v2, metrics + dashboards)*

- FastAPI instrumented with `prometheus-fastapi-instrumentator` → `/metrics`, exposing `http_requests_total{method,status,handler}` (status is the coarse `2xx`/`4xx`/`5xx` form, not exact codes), `http_request_duration_seconds_bucket{handler}` and the label-free high-resolution twin `http_request_duration_highr_seconds_bucket`, plus request/response size summaries
- Prometheus (`docker-compose.yml`, `:9090`) scrapes `host.docker.internal:8000` every 5s — `observability/prometheus/prometheus.yml`
- Grafana (`:3000`, admin/admin, anonymous viewer access enabled) with the Prometheus datasource auto-provisioned via `observability/grafana/provisioning/datasources/prometheus.yml` (now pinned to an explicit `uid: prometheus` so dashboard JSON can reference it reliably)
- **`observability/grafana/provisioning/dashboards/`** — new: `dashboards.yml` (file provider, watches this same directory) + `openscale-api.json`, an 8-panel "OpenScale API" dashboard (RED-method: rate, errors, duration) auto-loaded into Grafana's "OpenScale" folder on startup:
  - Stat row: API target up/down, total requests (5m), 5xx error rate (%), overall P95 latency
  - Request rate by endpoint, latency percentiles (P50/P95/P99) overall, P95 latency by endpoint, requests by status code — all timeseries
  - Verified against real traffic: generated requests across every endpoint, confirmed Prometheus scraped them and Grafana's query proxy returns correct values for every panel query

**Not yet done (see decision above for why):** OpenTelemetry/distributed tracing; no pipeline-level metrics (Kafka consumer lag, Silver/Gold job duration or data-quality scores aren't exported to Prometheus, only API request metrics are); no alerting rules.

**Gotcha hit getting here:** pinning an explicit `uid` on an already-provisioned datasource (rather than a fresh one) crashed Grafana on startup with `Datasource provisioning error: data source not found` — the old auto-generated uid was still referenced somewhere in Grafana's own SQLite state from before this uid was pinned. Fixed by dropping the local `grafana-data` Docker volume (pure infra state, nothing manually configured in it) and letting Grafana reprovision clean. Anyone pulling this repo fresh won't hit this — it only bites an existing, already-running local Grafana volume.
...
2. Add dropoff-zone (`DOLocationID`) stats to the Gold layer, alongside the existing pickup-zone stats.
3. Re-run Phase 1-style profiling across the full 40-month range to formally document quality-rate consistency and investigate the March–November 2025 data-quality anomaly found in the Silver quality reports (see Current State table).
4. Extend `replay.py` to loop across multiple/all days instead of one `--date` at a time, and give the streaming consumer somewhere to persist its windowed aggregates durably (today they only print to console and vanish when the process exits — no Gold-equivalent streaming output table yet).
5. Build actual Grafana dashboards against the Prometheus datasource (currently just wired up, nothing built).
6. Add demand-forecasting and congestion-prediction models alongside the current fare-prediction model, per `docs/plan.md`'s original ML scope.
7. Phase 12 (Benchmarking) is the only phase left entirely unstarted — now meaningfully attemptable since Phase 5 (PySpark) is real; the pandas-memory-wall measurement (Phase 5's trigger) is itself a first data point worth formalizing there.
8. Extend `ingestion/validator/schema_validator.py` to support multiple TLC schema eras (see the schema-drift constraint note above) if historical data before 2023-02 is ever needed — currently out of scope, worked around by date-range selection instead.
