...
def load_day(pickup_date: str) -> pd.DataFrame:
    # Filter in Spark (whole Silver lake, distributed) and only pull the one
    # day's worth of rows into pandas -- same big-in-Spark/small-in-pandas
    # pattern as the rest of the Phase 5 migration, not a full pd.concat().
    spark = get_spark("streaming-replay-load-day")
    try:
        df = load_silver_trips(spark)
        day_df = df.filter(F.to_date("tpep_pickup_datetime") == pickup_date)
        return day_df.toPandas().sort_values("tpep_pickup_datetime")
    finally:
        spark.stop()
