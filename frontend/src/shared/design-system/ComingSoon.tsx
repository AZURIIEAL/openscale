...
export function ComingSoon({ screenName, description }: ComingSoonProps) {
  return (
    <Panel className="flex flex-col items-start gap-2 p-6">
      <span className="os-font-mono text-[11px] uppercase tracking-[0.1em]" style={{ color: 'var(--ink-faint)' }}>
        Not built yet
      </span>
      <h2 className="os-font-display m-0 text-lg uppercase" style={{ color: 'var(--ink)' }}>
        {screenName}
      </h2>
      <p className="m-0 max-w-md text-sm" style={{ color: 'var(--ink-muted)' }}>
        {description}
      </p>
    </Panel>
  );
