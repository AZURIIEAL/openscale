...
def run(periods: list[tuple[int, int]] | None = None) -> Iterator[dict]:
    """Yields one result dict per period as it completes -- a generator (not
    a plain function returning a list) so a caller streaming this over a
    job queue/WebSocket sees real per-period progress, not one final burst
    once every period is done."""
    ...
    for year, month in target_periods:
        ...
        try:
            ...
            yield {"period": period, "status": "UNAVAILABLE", "reason": str(exc)}
            continue
        ...
        try:
            ...
            yield {"period": period, "status": "REJECTED", "reason": str(exc)}
            continue

        bronze_path = upload(df, year, month)
        yield {"period": period, "status": "OK", "bronze_path": str(bronze_path), "rows": len(df)}
